...
import matt.v1.compcache.showXAsPercent
...
fun experiments(fig: Figure, statusLabel: StatusLabel): List<Experiment> {
  ...
  fun varyingStimExp(
	...
	yMin = 0.0,
	yMax = 100.0,
	xMin = xMin.let { if (showXAsPercent) it*100.0 else it },
	xMax = xMax.let { if (showXAsPercent) it*100.0 else it },
	autoX = false,
	autoY = false,
	v2 = sequence {
	  val pop = Population(
		...
		).copyWith(ORIG_BUT_NEEDS_GPU)
	  )
	  val allSeries = mutableListsOf<Point>(3)
	  val (series1, series2, series3) = allSeries
	  val xRange = (xMin..xMax step xStep).toList()
	  xRange.forEachIndexed { xIndex, x ->
		statusLabel.counters["x:"] = xIndex + 1 to xRange.size
		...
		val responseWithDN = run {
		  val popR = MaybePreDNPopR(
			stim = stim,
			pop = pop,
			sigmaPooling = sqrt(5.0.toApfloat()),
		...
		series2 += Point(x = x, y = responseWithDN.R)
		series3 += Point(x = x, y = responseWithDN.G_S!!)
		yieldAll(allSeries.map {
		  replaceNextSeries(it.normalizeYToMax().let {
			if (showXAsPercent) {
			  it.showXAsPercent()
			} else {
			  it
  ...
  val baseExp = varyingStimExp(
	name = "3.B",
	title = "DN causes saturation with ↑ contrast",
	xlabel = "% Contrast",
	xStimOp = {
	  copy(
		a = it*popCfg.baseContrast, SF = (5.75)
	  )
	}
  )
  ...
  exps += varyingStimExp(
	...
	xMax = 50.0,
	xStimOp = {
	  val stimulus = copy(SF = 5.75)
	  stimulus withMask (stimulus.copy(
		a = (it*popCfg.baseContrast), f = stimulus.f.copy(tDegrees = orth(stimulus.f.tDegrees))
	  ))
	}
  )
  val f3dStep = 0.1
  exps += varyingStimExp(
	...
	xMax = 10.0,
	xStep = f3dStep,
	xStimOp = { copy(SF = 5.75, s = it) }
  )


  /*!!!!!!!!! NEED MAJOR REFACTORING AND CLEANUP BEFORE PASSING THIS POINT !!!!!!!!!*/
  /*!!!!!!!!! NEED MAJOR REFACTORING AND CLEANUP BEFORE PASSING THIS POINT !!!!!!!!!*/
  /*!!!!!!!!! NEED MAJOR REFACTORING AND CLEANUP BEFORE PASSING THIS POINT !!!!!!!!!*/
  /*!!!!!!!!! NEED MAJOR REFACTORING AND CLEANUP BEFORE PASSING THIS POINT !!!!!!!!!*/
  /*!!!!!!!!! NEED MAJOR REFACTORING AND CLEANUP BEFORE PASSING THIS POINT !!!!!!!!!*/
  ...
  val popGainBySize = exps.last().copy(
	name = "4.C", title = "↓ D.N. in ASD causes ↑ the absolute pop. gain (matches b. data)", xMax = 6.05, xMin = 1.55,
	series = listOf(td, ascC), ylabel = "Population Gain", autoY = true, normToMinMax = false, normToMax = false
  )
  exps += popGainBySize
  ...
  exps += baseExp.copy(
	name = "5.C", title = "dist. from center of attn. causes ↓ pop. gain and then ↑ (matches b. data)", xMin = 0.0,
	xMax = baseExp.attnX0DistMax.toDouble(), xOp = {
	...
	}, useAttentionMask = true, xStep = baseExp.f5cStep.toDouble(), xlabel = "Distance (Degrees)",
	ylabel = exps.last().ylabel, troughShift = true, autoY = true, series = listOf(td, ascC),

	normToMinMax = false, normToMax = false, popCfg = baseExp.popCfg.copy(
	  cellX0AbsMinmax = 15.0, conCircles = false
	)
  )


  exps += exps.last().copy(
	name = "5.D", title = "pop. gain gradient from attn increases with ASD symptomatology (↑c) (matches b. data)",
	...
	prob(
	  ft = cfgStim(
		cell = c, ftStim, popR = MaybePreDNPopR(
		  stim = ftStim, attention = outer.useAttentionMask, pop = outer.pop, uniformW = outer.uniformW,
		  rawInput = outer.rawInput, sigmaPooling = outer.pop.cfg.baseSigmaPooling, divNorm = outer.pop.cfg.baseDivNorm
		)()
	  ).R.toDouble(), preRI = cfgStim(
		cell = c, cfgStim = trialStim, popR = MaybePreDNPopR(
		  stim = trialStim, attention = outer.useAttentionMask, pop = outer.pop, uniformW = outer.uniformW,
		  rawInput = outer.rawInput, sigmaPooling = outer.pop.cfg.baseSigmaPooling, divNorm = outer.pop.cfg.baseDivNorm
		)()
  ...
  val stimOrientationXOpExample = exps.last().copy(
	...
	series = exps.last().series.map { (it as SeriesCfg).copy(yExtractCustom = PPC) }, autoY = false,
  ...
  val flatPriorPPC = flatPriorPPCbase.copy(yExtractCustom = {
	...
	)    /*(1..(if (poissonVar == NONE) 1 else 20)).map {*/    //	  println("gonna look for cell with X=${seriesStim.X0} Y=${seriesStim.Y0} t=${seriesStim.t}")
	/*decode*/    /*decodeBeforeGeoMean*/    /*warn("debugging 2 series commented out")*/
	val d1 = decode(
	  ftStim = seriesStim,
	...
	/*}.mean()*/    /*println("x=$x d=$d")*/
	d1 - d2 /*certainty = smaller width of PPC*/    /*d.toDouble()*/
  ...
  val preferedCellR = SeriesCfg(label = "Preferred Cell R", yExtractCustom = {
  ...
  val preferedCellG = SeriesCfg(label = "Preferred Cell G", yExtractCustom = {
	val y = lastPopR!!.m[outer.pop.centralCell]!!.G_S ?: 0.0
	if (x == itr.last()) lastPopR = null
	y
  })

  fun SFExciteSeries(sf: Double) = SeriesCfg(label = "SF:${sf}", yExtractCustom = {
  ...
  exps += exps.last().copy(name = "SF Peak Latency", popCfg = latestPop.copy(
	proportionalCellSigmas = true, cellSFmin = 2.0.toApfloat(), cellSFmax = 10.0.toApfloat(),
	cellSFstep = 2.0.toApfloat(), sampleStep = 0.5
